...
`packaging/aur/` holds the AUR source package (`pkgname=geary-hide-sidebar`,
built from the GitHub tag archive — note the archive top-dir is
`geary-hide-sidebar-module-$pkgver` because the *repo* name differs from the
*package* name). The package's interesting behavior is launcher injection, all
driven by `packaging/aur/inject.sh`:
...
create GitHub release → pin the source sha256 + sync `pkgver` into the PKGBUILD
→ publish to AUR via `KSXGitHub/github-actions-deploy-aur` (the `.install` is
shipped through the action's `assets:` input; `.SRCINFO` is regenerated by the
action). The in-tree PKGBUILD keeps `sha256sums=('SKIP')` between releases by
convention. AUR push is skipped if the `AUR_SSH_KEY` secret is unset. `ci.yml`
runs build+test on push/PR.
